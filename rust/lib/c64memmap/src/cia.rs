...
impl CIA {
  ...
  pub fn keydown(&mut self, index: u8) {
    ...
    let col = (index / 8) as usize;
    let shift = index % 8;
    let orig = self.keys[col];
    self.keys[col] = orig | (1 << shift);
  }
...
mod tests {
  ...
  fn keyboard() {
    let mut cia = CIA::new();
    cia.set_byte(0, 0b11111101);
    assert_eq!(cia.get_byte(1), 0b11111111);
    cia.keydown(10);
    assert_eq!(cia.get_byte(1), 0b11111011);
    cia.set_byte(0, 0b11111011);
    assert_eq!(cia.get_byte(1), 0b11111111);
    cia.keyup(10);
    cia.set_byte(0, 0b11111101);
    assert_eq!(cia.get_byte(1), 0b11111111);
    cia.keydown(56);
    cia.set_byte(0, 0b01111111);
    assert_eq!(cia.get_byte(1), 0b11111110);
  }
}